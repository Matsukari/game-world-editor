	...
	public abstract class BaseScene : Scene, IFinalRenderDelegate
  {
    ...
    static bool _needsFullRenderSizeForUi;
		...
		void SetupSceneSelector()
		{
			...
			sampleScenes.Row();
            sampleScenes.Add(new Label("Bases", new LabelStyle())).SetFillX().SetMinHeight(30);
            sampleScenes.Row();
			...
			// find every Scene with the BaseSceneAttribute and create a button for each one
			foreach (var type in GetTypesWithBaseSceneAttribute())
			{
				foreach (var attr in type.GetCustomAttributes(true))
				{
					if (attr.GetType() == typeof(BaseSceneAttribute))
					{
						var sampleAttr = attr as BaseSceneAttribute;
